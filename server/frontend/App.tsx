...
function Login(props: { onLogin: (user: User) => void }) {
  ...
  return <main class="auth-shell"><form class="panel auth-card" onSubmit={submit}><div class="brand auth-brand"><span class="brand-mark">✦</span><span><strong>Mugen</strong><small>History &amp; Stars</small></span></div><span class="eyebrow">{authorize ? "EXTENSION LOGIN" : "WELCOME BACK"}</span><h1>ログイン</h1><p>管理者から発行されたアカウントでログインしてください。</p><label>メールアドレス<input type="email" required value={email()} onInput={(event) => setEmail(event.currentTarget.value)} /></label><label>パスワード<input type="password" required value={password()} onInput={(event) => setPassword(event.currentTarget.value)} /></label><Show when={error()}><div class="error-message">{error()}</div></Show><button class="primary-button auth-submit" disabled={busy()}>{busy() ? "ログイン中…" : "ログイン"}</button></form></main>;
}
...
function Header(props: { page: Page; user: User }) { return <header class="topbar"><div><span class="breadcrumb">Workspace / </span><strong>{props.page === "dashboard" ? "Dashboard" : "Account"}</strong></div><div class="user-chip"><span class="avatar">{props.user.displayName.slice(0, 2)}</span><span>{props.user.displayName}</span></div></header>; }
...
function Account(props: { user: User; onUpdate: (user: User) => void }) {
  ...
  return <><div class="page-heading"><div><span class="eyebrow">ACCOUNT</span><h1>アカウント管理</h1><p>プロフィール情報を更新できます。</p></div></div><form class="panel account-form" onSubmit={save}><div class="panel-heading"><div><span class="eyebrow">PROFILE</span><h2>プロフィール</h2></div></div><label>表示名<input required value={name()} onInput={(event) => setName(event.currentTarget.value)} /></label><label>メールアドレス<input required type="email" value={email()} onInput={(event) => setEmail(event.currentTarget.value)} /></label><div class="form-actions"><button class="primary-button">変更を保存</button></div><Show when={saved()}><p class="form-hint">{saved()}</p></Show></form><section class="content-grid account-grid"><article class="panel"><div class="panel-heading"><div><span class="eyebrow">SYNC SETTINGS</span><h2>同期設定</h2></div></div><label>同期 URL<input value={`${location.origin}/v1/sync`} readonly /></label><p class="form-hint">同期秘密鍵は不要です。拡張機能ではこのアカウントでログインしてください。</p></article><article class="panel danger-panel"><div class="panel-heading"><div><span class="eyebrow">ACCOUNT STATUS</span><h2>アカウント状態</h2></div></div><p>状態: <strong>{props.user.status === "active" ? "有効" : "無効"}</strong></p><p>プラン変更やパスワード再設定は管理者に依頼してください。</p></article></section></>;
}
