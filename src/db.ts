  ...
  return new Promise((resolve, reject) => {
    ...
    request.onupgradeneeded = () => {
      ...
      if (!db.objectStoreNames.contains('visits')) {
        ...
        if (!store.indexNames.contains('syncId')) store.createIndex('syncId', 'syncId', { unique: true })
        const cursorRequest = store.openCursor()
        cursorRequest.onsuccess = () => {
          const cursor = cursorRequest.result
          if (!cursor) return
          const value = cursor.value as Partial<HistoryEntry>
          cursor.update({
            ...value,
            syncId: value.syncId ?? uuid(),
            updatedAt: value.updatedAt ?? value.visitedAt ?? Date.now(),
            updatedBy: value.updatedBy ?? 'migration',
          })
          cursor.continue()
        }
      }
      ...
      if (!db.objectStoreNames.contains('bookmarks')) {
        ...
        cursorRequest.onsuccess = () => {
          const cursor = cursorRequest.result
          if (!cursor) return
          const value = cursor.value as Partial<BookmarkEntry>
          cursor.update({
            ...value,
            syncId: value.syncId ?? uuid(),
      ...
      if (!db.objectStoreNames.contains('sync_outbox')) {
        const store = db.createObjectStore('sync_outbox', { keyPath: 'operationId' })
        store.createIndex('itemId', 'itemId', { unique: false })
        store.createIndex('clientUpdatedAt', 'clientUpdatedAt', { unique: false })
      }
...
export async function addVisit(entry: Omit<HistoryEntry, 'id' | 'syncId' | 'updatedAt' | 'updatedBy'>): Promise<void> {
  const info = await getSyncInfo()
  const db = await openDB()
  const stores = info ? ['visits', 'sync_outbox'] : ['visits']
  const tx = db.transaction(stores, 'readwrite')
  const store = tx.objectStore('visits')
  const lastCursor = await requestResult<IDBCursorWithValue | null>(store.index('visitedAt').openCursor(null, 'prev'))
  ...
  if (last && last.url === entry.url) {
    ...
    if (info) {
      putOutbox(tx.objectStore('sync_outbox'), 'history', 'delete', {
        syncId: last.syncId,
        updatedAt,
...
export async function deleteVisit(id: number): Promise<void> {
  ...
  if (existing) {
    store.delete(id)
    if (info) putOutbox(tx.objectStore('sync_outbox'), 'history', 'delete', {
...
export async function clearAllVisits(): Promise<void> {
  ...
  if (info) {
    ...
    for (const entry of entries) {
      putOutbox(outbox, 'history', 'delete', {
        syncId: entry.syncId,
...
export async function getVisitsPage(page: number, perPage: number): Promise<{ items: HistoryEntry[]; total: number }> {
  const db = await openDB()
  const tx = db.transaction('visits', 'readonly')
  const store = tx.objectStore('visits')
  ...
  const items = await new Promise<HistoryEntry[]>((resolve, reject) => {
    ...
    req.onsuccess = () => {
      ...
      if (results.length < perPage) { results.push(cursor.value); cursor.continue(); return }
      resolve(results)
    }
...
export async function searchVisitsPage(query: string, page: number, perPage: number): Promise<{ items: HistoryEntry[]; total: number }> {
  const all = await getAllVisits()
  const q = query.toLowerCase()
  const filtered = all.filter(e => e.url.toLowerCase().includes(q) || e.title.toLowerCase().includes(q))
...
export async function removeBookmark(id: number): Promise<void> {
  ...
  if (existing) {
    store.delete(id)
    if (info) putOutbox(tx.objectStore('sync_outbox'), 'bookmark', 'delete', {
      syncId: existing.syncId,
      updatedAt: nextUpdatedAt(existing.updatedAt),
      updatedBy: info.deviceId,
    }, info)
...
export async function updateBookmark(id: number, changes: Partial<Pick<BookmarkEntry, 'name' | 'url'>>): Promise<void> {
  const info = await getSyncInfo()
  const db = await openDB()
  const tx = db.transaction(info ? ['bookmarks', 'sync_outbox'] : ['bookmarks'], 'readwrite')
  const store = tx.objectStore('bookmarks')
...
export async function getBookmarkByUrl(url: string): Promise<BookmarkEntry | undefined> {
  ...
  const result = await requestResult<BookmarkEntry | undefined>(tx.objectStore('bookmarks').index('url').get(url))
  db.close()
  return result
}
...
export async function getOutbox(limit = 100): Promise<OutboxEntry[]> {
  ...
  const result = await requestResult<OutboxEntry[]>(tx.objectStore('sync_outbox').getAll(undefined, limit))
  db.close()
  return result
}
...
export async function getSyncState(): Promise<SyncState> {
  ...
  const cursor = await requestResult<{ key: string; value: unknown } | undefined>(store.get('cursor'))
  const bootstrap = await requestResult<{ key: string; value: unknown } | undefined>(store.get('bootstrap'))
  const pendingDeletionIds = await requestResult<{ key: string; value: unknown } | undefined>(store.get('pendingDeletionIds'))
  db.close()
  return {
    cursor: typeof cursor?.value === 'number' ? cursor.value : 0,
    bootstrap: bootstrap?.value === 'queued' || bootstrap?.value === 'complete' ? bootstrap.value : 'pending',
    pendingDeletionIds: Array.isArray(pendingDeletionIds?.value) && pendingDeletionIds.value.every((value) => typeof value === 'number')
...
export async function enqueueBootstrap(): Promise<boolean> {
  ...
  const state = await requestResult<{ key: string; value: unknown } | undefined>(tx.objectStore('sync_state').get('bootstrap'))
...
export async function applyRemoteChanges(changes: RemoteChange[]): Promise<void> {
  ...
  for (const change of changes) {
    ...
    const current = await requestResult<HistoryEntry | BookmarkEntry | undefined>(store.index('syncId').get(change.itemId))
...
function isRemoteNewer(change: RemoteChange, current: HistoryEntry | BookmarkEntry): boolean {
  return change.clientUpdatedAt > current.updatedAt ||
...
function normalizeRemotePayload(change: RemoteChange, payload: Record<string, unknown>): Record<string, unknown> {
  if (!requiredString(payload.url) || !requiredString(payload.favicon)) throw new Error('Invalid sync payload')
  if (change.kind === 'history') {
    if (!requiredString(payload.title) || typeof payload.visitedAt !== 'number') throw new Error('Invalid sync payload')
    return {
  ...
  if (!requiredString(payload.name) || typeof payload.createdAt !== 'number') throw new Error('Invalid sync payload')
  return {
    syncId: change.itemId,
    url: payload.url,
    name: payload.name,
...
export async function commitSyncBatch(operationIds: string[], cursor: number, pendingDeletionIds: number[]): Promise<void> {
  const db = await openDB()
