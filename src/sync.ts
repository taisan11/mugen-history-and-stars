...
export async function getSyncSettings(): Promise<SyncSettings> {
  const data = await browser.storage.local.get(['syncUrl', 'syncSecret', 'syncDeviceId', 'syncEnabled']) as Partial<SyncSettings>
  return {
...
export async function saveSyncSettings(syncUrl: string, syncSecret: string): Promise<void> {
  ...
  if (!validSecret(syncSecret)) throw new Error(`同期秘密鍵は${MIN_SECRET_LENGTH}文字以上で指定してください`)
...
async function deriveAuthKey(secret: string): Promise<CryptoKey> {
  ...
  const derive = (label: string, keyAlgorithm: AesKeyGenParams | HmacKeyGenParams, usages: KeyUsage[]) =>
...
async function authorizationToken(authKey: CryptoKey): Promise<string> {
  const signature = await crypto.subtle.sign('HMAC', authKey, new TextEncoder().encode('mugen-history-sync-auth-v1'))
...
function isResponse(value: unknown): value is SyncResponse {
  ...
  return typeof response.cursor === 'number' && Number.isSafeInteger(response.cursor) &&
    Array.isArray(response.changes) && response.changes.length <= RESPONSE_PAGE_SIZE
}

function isResponseChange(value: unknown): value is SyncResponseChange {
  if (typeof value !== 'object' || value === null) return false
  const change = value as Partial<SyncResponseChange>
  return typeof change.operationId === 'string' && typeof change.itemId === 'string' &&
    (change.kind === 'history' || change.kind === 'bookmark') &&
    (change.operation === 'upsert' || change.operation === 'delete') &&
    typeof change.clientUpdatedAt === 'number' && Number.isSafeInteger(change.clientUpdatedAt) &&
    typeof change.deviceId === 'string' &&
    (change.deletionId === undefined || (typeof change.deletionId === 'number' && Number.isSafeInteger(change.deletionId)))
}
...
async function performSync(): Promise<SyncResult> {
  ...
  if (!validSecret(settings.syncSecret)) throw new Error(`同期秘密鍵は${MIN_SECRET_LENGTH}文字以上で指定してください`)
  ...
  while (shouldContinue) {
    ...
    await commitSyncBatch(outbox.map((entry) => entry.operationId), response.cursor, pendingDeletionIds)
    ...
    shouldContinue = response.changes.length === RESPONSE_PAGE_SIZE || remaining.length > 0 || pendingDeletionIds.length > 0
  }
